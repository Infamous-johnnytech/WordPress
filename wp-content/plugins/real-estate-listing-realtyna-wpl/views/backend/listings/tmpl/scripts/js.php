...
<script type="text/javascript">
	...
	{
        var message = '<?php echo addslashes(__("Are you sure to revert this expired property?", "real-estate-listing-realtyna-wpl")); ?>';
        var confirmation = confirm(message);

        if(!confirmation) return
		
		var element_id = wplj("#" + id).data('id');		
		
        var request_str = "wpl_format=b:listings:ajax&wpl_function=revert_expired_property&pid="+element_id+"&_wpnonce=<?php echo $this->nonce; ?>";		
        ...
        ajax.success(function(data)
        {
			...
			if(data.success == 1)
            {
				wplj("#pmanager_expired_status" + element_id).fadeOut(300, function() { wplj(this).remove(); });//.remove();
            }else
    ...
    function trash_property(pid)
    {
        ...
        ajax.success(function(data)
        {
            if(data.success == 1)
            {
                ...
                else wplj("#pmanager_trash"+pid+" span").text("<?php echo addslashes(__('Trash', 'real-estate-listing-realtyna-wpl')); ?>");
            }
            else if(data.success != 1)
            {
