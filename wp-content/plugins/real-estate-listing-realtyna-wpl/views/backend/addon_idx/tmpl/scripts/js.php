...
<script type="text/javascript">
        ...
        wplj.ajax(
            {
                ...
                success: function (data) {
                    ...
                    mlsProviders += '<td class="price_total">' + data.message.price + '$ <?php echo __("Per Month"); ?> ' + ' + ' + data.message.one_time_price + '$ <?php echo __("One-time fee"); ?> ' + '<input type="hidden" value="'+50 +'"></td>';
                    mlsProviders += '</tr>';
    ...
    function wpl_idx_generate_config_form(property_types)
    {
        ...
        config_form += '<?php echo __('Download Text AND Images to your server (Please note, this will increase your hosting load)', 'real-estate-listing-realtyna-wpl')?>';
        config_form +='</div>';
        config_form +='<div class="wpl-idx-form-checkbox">';
        config_form +='<input id="text_download_checkbox" type="checkbox" class="yesno" checked="checked">';
        config_form += '<?php echo __('Download ONLY Text to your servers (Preferred for IDX subscribers)', 'real-estate-listing-realtyna-wpl')?>'; 
        config_form +='</div>';
        config_form +='</div>';
        config_form +='<div class="wpl-small-12 wpl-medium-6 wpl-large-6 wpl-column">';
        ...
        wplj.ajax(
            {
                ...
                success: function (data) {
                       if( data.status == 200) {
                        value = data.message;
                        if(value.configStatus === 'Active') action = '<a href="#" onclick="wpl_idx_delete(0);"><?php echo __('Cancel Subscription','real-estate-listing-realtyna-wpl'); ?></a>';
    ...
    function wpl_idx_delete(confirmed)
    {
        if (!confirmed)
        {
            message = "<?php echo __('Are you sure you want to cancel your subscription?', 'real-estate-listing-realtyna-wpl'); ?>";
            ...
            wplj.ajax(
                {
                    ...
                    success: function (data) {
                        if(data.status === 200 || data.status === 201){
                            wplj('.wpl-idx-wizard-main #wpl-idx-setting-table .wpl-idx-addon-table-row .actions').html('<?php echo __('The subscription got canceled', 'real-estate-listing-realtyna-wpl')?>');
                            wplj('.wpl-idx-wizard-main #wpl-idx-setting-table .wpl-idx-addon-table-row .status').html('<?php echo __('Pending', 'real-estate-listing-realtyna-wpl')?>');
                        }
                        else{
