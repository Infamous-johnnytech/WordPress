...
if($show == 'simple_location_text' and !$done_this)
{
	...
	for($i=1; $i<=2; $i++)
	{
		...
		$html .= '<option value="-1">'.__((trim($location_settings['location'.$i.'_keyword']) != '' ? $location_settings['location'.$i.'_keyword'] : 'Select'), 'real-estate-listing-realtyna-wpl').'</option>';
	...
	$html .= '<label class="wpl_search_widget_location_level_label" for="sf'.$widget_id.'_locationtextsearch">'.__('Location Text', 'real-estate-listing-realtyna-wpl').'</label>';
	$html .= '<input class="wpl_search_widget_'.$field['id'].'_location_text" value="'.esc_attr($current_values['locationtextsearch']).'" name="sf'.$widget_id.'_locationtextsearch" id="sf'.$widget_id.'_locationtextsearch" placeholder="'.__($placeholder, 'real-estate-listing-realtyna-wpl').'" type="text" />';
	$html .= '</div>';
