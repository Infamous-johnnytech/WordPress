...
if($format == 'select' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
    {
        ...
        {
            $query .= " AND (`".$table_column."` = '".$value."' OR `additional_agents` LIKE '%,".$value.",%')";
        }
        else $query .= " AND `".$table_column."` = '".$value."'";
    }
...
elseif($format == 'multiselect' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
    {
        $query .= " AND (`".$table_column."` = '".$value."' OR `".$table_column."` LIKE '".$value.",%' OR `".$table_column."` LIKE '%,".$value."' OR `".$table_column."` LIKE '%,".$value.",%')";
    }
...
elseif($format == 'notmultiple' and !$done_this)
{
    if(!($value == '' or $value == ','))
    {
        ...
        if($value != '')
        {
            ...
            $query .= " AND `".$table_column."` NOT IN (".trim($value_str, ', ').")";
        }
    }
	...
	if(!($value == '' or $value == '-1' or $value == ','))
	{
		...
		if($value != '')
        {
            ...
            foreach($values_ex as $value_ex)
            {
            	...
            	$q .= "(`".$table_column."` = '".$value_ex."' OR `".$table_column."` LIKE '".$value_ex.",%' OR `".$table_column."` LIKE '%,".$value_ex."' OR `".$table_column."` LIKE '%,".$value_ex.",%') OR ";
            }
...
elseif($format == 'notmultiselectmultiple' and !$done_this)
{
	if(!($value == '' or $value == '-1' or $value == ','))
	{
		...
		if($value != '')
        {
            ...
            {
            	$value_ex = trim($value_ex);
            	$q  .= "(`".$table_column."_options` NOT LIKE '".$value_ex."' AND `"
            	    .$table_column."_options` NOT LIKE '%,".$value_ex.",%' AND `"
            	    .$table_column."_options` NOT LIKE '".$value_ex.",%' AND `"
            	    .$table_column."_options` NOT LIKE '%,".$value_ex."' AND `"
            	    .$table_column."_options` NOT LIKE ',".$value_ex.",') AND ";
            }
            ...
            $q = rtrim($q, ' AND ');
            $query .= " AND ($q OR `".$table_column."_options` IS NULL)";
        }
	}
...
elseif($format == 'betweenmmunit' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
        ...
        if($si_value_min != 0) $query .= " AND `".$table_column."_si` >= '".$si_value_min."'";
		if($si_value_max != 0) $query .= " AND `".$table_column."_max_si` <= '".$si_value_max."'";
	}
...
elseif($format == 'mmunit' and !$done_this)
{
	if($value != '-1' and trim($value) != '')
	{
		...
		{
			// min [property] max
			$q = "(`".$table_column."_si` >= '".$si_value_min."' AND `".$table_column."_max_si` <= '".$si_value_max."') OR ";

			// [min property max]
			$q .= "(`".$table_column."_si` <= '".$si_value_min."' AND `".$table_column."_max_si` >= '".$si_value_max."') OR ";

			// min [property max]
			$q .= "(`".$table_column."_si` >= '".$si_value_min."' AND `".$table_column."_si` <= '".$si_value_max."' AND `".$table_column."_max_si` >= '".$si_value_max."') OR ";

			// [min property] max
			$q .= "(`".$table_column."_si` <= '".$si_value_min."' AND `".$table_column."_max_si` >= '".$si_value_min."' AND `".$table_column."_max_si` <= '".$si_value_max."')";
...
{
	if($value != '-1' and trim($value) != '') $query .= " AND `".$table_column."` <= ".$value." AND `".$table_column."_max` >= ".$value;
	$done_this = true;
}