...
class wpl_settings_controller extends wpl_controller
{
    ...
    private function add_sample_properties()
    {
    	...
    	for($i = 0; $i < 6; $i++)
    	{
			...
			foreach($fields as $field)
			{
				...
				elseif($field->type == 'neighborhood')
				{
					...
					if($value == 1)
					{
                        ...
                        if($field->table_name == 'wpl_properties2') $query2 .= "`{$field->table_column}_distance` = '{$dist}', `{$field->table_column}_distance_by` = '{$dist_by}', ";
                        else $query .= "`{$field->table_column}_distance` = '{$dist}', `{$field->table_column}_distance_by` = '{$dist_by}', ";
					}
				}

				if($field->table_name == 'wpl_properties2') $query2 .= "`{$field->table_column}` = '{$value}', ";
			...
			if(trim($query2, ', ')) wpl_db::q("UPDATE `#__wpl_properties2` SET ".trim($query2, ', ')." WHERE `id` = '$pid'");
