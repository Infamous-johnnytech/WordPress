...
class wpl_images
{
    ...
    public static function resize_image($source, $dest, $width, $height, $crop = 0)
    {
		...
		$get_webp_image = wpl_settings::get('image_webp_optimization_setting');
		
		if ( wpl_global::check_addon('optimizer') and !empty( $get_webp_image ) ){ 
		
			if ( method_exists( 'wpl_addon_optimizer' , 'image_convert_to_webp' ) ){
			
				$dest = wpl_addon_optimizer::image_convert_to_webp( $extension, $dest );
    ...
    public static function add_watermark_image($source, $dest, $options = '') 
    {
        ...
        if($options['status'] != 1) return NULL;
        ...
        if(!wpl_file::exists($watermark)) return NULL;
