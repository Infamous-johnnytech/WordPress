...
class wpl_property_finalize
{
    ...
    public function start()
    {
        ...
        foreach($this->q as $column=>$value)
        {
            ...
            else $u .= "`".$column."`='".wpl_db::escape($value)."',";
        }
        ...
        // Properties2 Table
        if($this->db->select("SELECT `id` FROM `#__wpl_properties2` WHERE `id`='".$this->property_id."'", 'loadResult'))
        {
            // Update

            $u2 = '';
            foreach($this->q2 as $column=>$value) $u2 .= "`".$column."`='".$value."',";
