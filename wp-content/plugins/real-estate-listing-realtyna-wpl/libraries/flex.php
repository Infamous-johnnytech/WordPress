...
class wpl_flex
{
	...
	public static function run_dbst_type_queries($dbst_id, $dbst_type, $dbst_kind, $query_type = 'add', $table = NULL)
	{
		...
		foreach($queries as $query)
		{
            ...
            $default_value = (isset($options['default_value']) ? $options['default_value'] : 0);
    ...
    {
        if(is_array($kind)) return wpl_flex::get_fields(NULL, NULL, NULL, NULL, NULL, "AND `type`='feature' AND `enabled`>='1' AND `kind` IN (".implode(',', $kind).")");
        else return wpl_flex::get_fields(NULL, NULL, NULL, NULL, NULL, "AND `type`='feature' AND `enabled`>='1' AND `kind`='$kind'");
    }
    ...
    public static function change_storage($dbst_id, $new_storage)
    {
        ...
        {
            wpl_db::q("UPDATE `#__".$new_storage."` SET `".$column."`=(SELECT `".$column."` FROM `#__".$previous_storage."` WHERE `id`=`#__".$new_storage."`.`id`)");
        }
        ...
        {
            wpl_db::q("DELETE FROM `#__wpl_sort_options` WHERE `field_name`='p.".$dbst->table_column."'");
        }
    }
