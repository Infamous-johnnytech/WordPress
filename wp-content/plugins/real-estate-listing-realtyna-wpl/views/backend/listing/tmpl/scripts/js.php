...
function ajax_multilingual_save(field_id, lang, value, item_id)
{
	...
	var request_str = 'wpl_format=b:listing:ajax&wpl_function='+wpl_function+'&dbst_id='+field_id+'&value='+encodeURIComponent(value)+'&item_id='+item_id+'&lang='+lang+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function ajax_save(table_name, table_column, value, item_id, field_id, form_element_id, wpl_function)
{
	...
	var request_str = 'wpl_format=b:listing:ajax&wpl_function='+wpl_function+'&table_name='+table_name+'&table_column='+table_column+'&value='+encodeURIComponent(value)+'&item_id='+item_id+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function item_save(value, item_id, field_id, item_type, item_cat, item_extra1, item_extra2, item_extra3, form_element_id, wpl_function)
{
	...
	var request_str = 'wpl_format=b:listing:ajax&wpl_function='+wpl_function+'&value='+encodeURIComponent(value)+'&item_id='+item_id+'&item_type='+item_type+'&item_cat='+item_cat+'&item_extra1='+item_extra1+'&item_extra2='+item_extra2+'&item_extra3='+item_extra3+'&kind=<?php echo $this->kind; ?>&_wpnonce=<?php echo $this->nonce; ?>';
...
function wpl_neighborhood_distance_type_select(table_name, table_column, value, item_id, field_id, form_element_id)
{
	if(wplj('#wpl_c_'+field_id+'_distance').val() == '')
	{
		...
		wpl_alert("<?php echo __("Please enter distance first!", 'real-estate-listing-realtyna-wpl'); ?>");
	}
	else
	{
...
function number_to_th(number)
{
	...
	else if(number == 1) return "<?php echo __('First', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 2) return "<?php echo __('Second', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 3) return "<?php echo __('Third', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 4) return "<?php echo __('Fourth', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 5) return "<?php echo __('Fifth', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 6) return "<?php echo __('Sixth', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 7) return "<?php echo __('Seventh', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 8) return "<?php echo __('Eighth', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 9) return "<?php echo __('Nineth', 'real-estate-listing-realtyna-wpl'); ?>";
	else if(number == 10) return "<?php echo __('Tenth', 'real-estate-listing-realtyna-wpl'); ?>";
}
