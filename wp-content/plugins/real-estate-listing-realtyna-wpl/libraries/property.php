...
class wpl_property
{
     ...
     * @return boolean
     */
    public static function revert($property_id , $trigger_event = true)
    {
        /** first validation **/
        if(!trim($property_id) or !in_array($status, array(0, 1))) return false;
    ...
    public static function clone_listing($property_id)
    {
        ...
        foreach($listing_data as $key=>$value)
        {
            ...
            if(in_array($key, $wpl_properties_columns)) $q .= "`$key`='$value', ";
            elseif(in_array($key, $wpl_properties2_columns)) $q2 .= "`$key`='$value', ";
        }

        $q .= trim($q, ', ');
        if(trim($q)) wpl_db::q("UPDATE `#__wpl_properties` SET $q WHERE `id`='$clone_id'");

        $q2 .= trim($q2, ', ');
        if(trim($q2)) wpl_db::q("UPDATE `#__wpl_properties2` SET $q2 WHERE `id`='$clone_id'");
