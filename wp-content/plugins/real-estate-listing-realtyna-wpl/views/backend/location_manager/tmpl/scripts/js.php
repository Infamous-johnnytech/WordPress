...
function wpl_set_enabled_location(location_id, enabeled_status)
{
	if(!location_id)
	{
		wpl_show_messages("<?php echo addslashes(__('Invalid Location', 'real-estate-listing-realtyna-wpl')); ?>", '.wpl_location_list .wpl_show_message');
		return false;
	}
	...
	request_str = 'wpl_format=b:location_manager:ajax&wpl_function=set_enabled_location&location_id='+location_id+'&enabeled_status='+enabeled_status+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function wpl_generate_modify_page(level, parent, location_id)
{
	...
	request_str = 'wpl_format=b:location_manager:ajax&wpl_function=generate_modify_page&level='+level+'&parent='+parent+'&location_id='+location_id+'&_wpnonce=<?php echo $this->nonce; ?>';
	...
	wplj.ajax(
	{
		...
		{
			wpl_show_messages('<?php echo addslashes(__('Error Occured.', 'real-estate-listing-realtyna-wpl')); ?>', '.wpl_location_list .wpl_show_message', 'wpl_red_msg');
...
function wpl_generate_params_page(level, location_id)
{
	...
	request_str = 'wpl_format=b:location_manager:ajax&wpl_function=generate_params_page&level='+level+'&location_id='+location_id+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function wpl_ajax_modify_location(level, parent, location_id)
{
	...
	request_str = 'wpl_format=b:location_manager:ajax&wpl_function=save_location&name='+name+'&abbr='+abbr+'&level='+level+'&parent='+parent+'&location_id='+location_id+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function wpl_remove_location(level, location_id, confirmed)
{
	...
	{
		wpl_show_messages("<?php echo addslashes(__('Invalid Location or level', 'real-estate-listing-realtyna-wpl')); ?>", '.wpl_location_list .wpl_show_message');
		return false;
	}
	
	if(!confirmed)
	{
		message = "<?php echo addslashes(__('Are you sure you want to remove this item?', 'real-estate-listing-realtyna-wpl')); ?>&nbsp;(<?php echo addslashes(__('Level', 'real-estate-listing-realtyna-wpl')); ?>:"+level+", <?php echo addslashes(__('ID', 'real-estate-listing-realtyna-wpl')); ?>:"+location_id+")&nbsp;<?php echo addslashes(__('All related items will be removed.', 'real-estate-listing-realtyna-wpl')); ?>";
		message += '<span class="wpl_actions" onclick="wpl_remove_location(\''+level+'\',\''+location_id+'\', 1);"><?php echo addslashes(__('Yes', 'real-estate-listing-realtyna-wpl')); ?></span>&nbsp;<span class="wpl_actions" onclick="wpl_remove_message();"><?php echo addslashes(__('No', 'real-estate-listing-realtyna-wpl')); ?></span>';
	...
	request_str = 'wpl_format=b:location_manager:ajax&wpl_function=delete_location&location_id='+location_id+'&level='+level+'&wpl_confirmed='+confirmed+'&_wpnonce=<?php echo $this->nonce; ?>';
...
function wpl_setting_save(setting_id, setting_name, setting_value, setting_category)
{
	...
	var request_str = 'wpl_format=b:settings:ajax&wpl_function=save&setting_name='+setting_name+'&setting_value='+setting_value+'&setting_category='+setting_category+'&_wpnonce=<?php echo wpl_security::create_nonce('wpl_settings'); ?>';
