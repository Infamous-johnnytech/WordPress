...
{
	var filename = wplj("#<?php echo $this->html_element_id; ?>").val();
	...
	{
		wpl_show_messages('<?php echo esc_js(__('File extension does not match.', 'real-estate-listing-realtyna-wpl')); ?>', '<?php echo $this->html_path_message; ?>', 'wpl_red_msg');
		return false;
	}

	var ajax_loader_element = '<?php echo $this->html_ajax_loader; ?>';
	wplj(ajax_loader_element).html('<img src="<?php echo wpl_global::get_wpl_asset_url('img/'.$this->img_ajax_loader); ?>" />');

	var request_str = '<?php echo $this->request_str; ?>';
